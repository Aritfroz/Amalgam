...
public class EntityPeridot extends EntityAmalgamGem implements IInventoryChangedListener, INpc {
	...
	public void checkSurroundings(World world, BlockPos pos) {
		if (!world.isRemote) {
			InjectorResult result = ((this.lastCheckPos != null && this.getDistanceSq(this.lastCheckPos) > 32.0F) || this.lastCheckPos == null || this.world.getTotalWorldTime() - this.lastCheckTime > 2400) ? InjectorResult.create(world, pos, false) : this.lastResult;
	...
	public boolean canPickupItem(Item item) {
		if (this.isFarmer()) {
			return item instanceof ItemSeeds || item instanceof ItemSeedFood || item instanceof ItemFood || item == Items.WHEAT;
		}
		return false;
	}
